...
public class Vampire extends objetMobile_abstrait implements Ennemis_interface{

    
public Vampire(int x) {
           // initialiser les coordonnées 
            this.x = x;
            this.y = 0;
            Random r = new Random();
            int num_vampire = r.nextInt(4) + 1;
            // affecter l'image d'un vampire a l'imageicon v
		ImageIcon v = new ImageIcon(this.getClass().getResource("images/ennemis/vampire"+num_vampire+".png"));
...
public Vampire cloneToi(){
            Vampire clone = new Vampire(this.x);
             Random r = new Random();
             int X = r.nextInt(730);
             clone.setX(X);
             return clone;
